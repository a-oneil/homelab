...
├── ui.py                    # Colors (class C), pick_option, pick_multi, confirm, prompt_text, bar_chart
...
│   ├── unraid.py            # Dashboard, Docker (stats, inspect, update, images, compose), VMs, parity check, notification center, user scripts, logs, SMART, VSCode, media scan
...
Plugins are registered in `main.py` in the `PLUGINS` list (19 plugins total). Adding a new plugin:
...
- **containerupdates.py** — Pulls latest Docker image digests over SSH, compares against running containers, shows outdated vs current
...
- `ssh_run(cmd, capture=True)` — runs SSH command, returns CompletedProcess
- TTY commands (nano, shell, logs): use `subprocess.run(["ssh", "-t", get_host(), cmd])`
