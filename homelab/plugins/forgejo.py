...
class ForgejoPlugin(Plugin):
    ...
    def get_actions(self):
        return {
            ...
            "Forgejo Pull Requests": ("forgejo_prs", _list_pull_requests),
        }
...
def _create_repo():
    ...
    if result:
        ...
        log_action("Forgejo Create Repo", full_name)
...
def _clone_repo(repo):
    ...
    try:
        ...
        if result.returncode == 0:
            log_action("Forgejo Clone Repo", f"{full_name} → {target}")
...
def _list_pull_requests():
    ...
    for repo in repos["data"]:
        full_name = repo.get("full_name", "")
        prs = _api(f"/repos/{full_name}/pulls?state=open&limit=10")
...
def _pr_detail(pr):
    ...
    if aidx == 2:
        ...
        if confirm(f"Merge PR #{num} '{title}'?", default_yes=False):
            result = _api(
                f"/repos/{repo}/pulls/{num}/merge", method="POST",
                data={"Do": "merge", "merge_message_field": title})
            if result is not None:
                log_action("Forgejo Merge PR", f"{repo} #{num}")
        ...
        if confirm(f"Close PR #{num} without merging?", default_yes=False):
            result = _api(
                ...
                data={"state": "closed"})
            if result is not None:
                log_action("Forgejo Close PR", f"{repo} #{num}")
...
def _repo_ci_runs(full_name):
    """Show recent CI/Actions runs for a repository."""
    runs = _api(f"/repos/{full_name}/actions/runs?limit=20")
    if not runs:
        info("No CI runs found (Actions may not be configured).")
        input(f"\n  {C.DIM}Press Enter to continue...{C.RESET}")
        return
    ...
    print(f"\n  {C.BOLD}CI Runs — {full_name}{C.RESET}\n")
...
def _repo_webhooks(full_name):
    ...
    while True:
        hooks = _api(f"/repos/{full_name}/hooks")
        if hooks is None:
...
def _create_webhook(full_name):
    ...
    events_map = {
        0: ["push"],
        1: ["push", "pull_request"],
        2: ["create", "delete", "fork", "push", "issues", "pull_request", "release"],
    }
    ...
    if result:
        log_action("Forgejo Create Webhook", f"{full_name} → {url}")
...
def _webhook_actions(full_name, hook):
    ...
    if aidx == 3:
        ...
        if result:
            ...
            log_action("Forgejo Webhook Toggle", f"{full_name} #{hid} {state_str}")
        ...
        if confirm(f"Delete webhook #{hid}?", default_yes=False):
            ...
            if result is not None:
                log_action("Forgejo Delete Webhook", f"{full_name} #{hid}")
        ...
        if idx == 3:
            ...
            if confirm(f"Make {full_name} {new_label}?", default_yes=False):
                ...
                if result:
                    ...
                    log_action("Forgejo Repo Visibility", f"{full_name} → {new_label}")
            ...
            if new_desc is not None:
                ...
                if result:
                    ...
                    log_action("Forgejo Repo Description", full_name)
            ...
            if confirm(f"{action.title()} {full_name}?", default_yes=False):
                ...
                if result:
                    ...
                    log_action(f"Forgejo Repo {action.title()}", full_name)
